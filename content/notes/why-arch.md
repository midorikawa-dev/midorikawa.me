...
ただ、私のマルチプラットフォームは Linux と Android だ。

一番揃えたい SDK、NDK、Emulator は、さっき書いたとおり Nix が一番苦手な種類のバイナリで、Windows に至っては Nix は WSL 越しにしか存在できない。

そして私のクロスプラットフォームは、rustup と Gradle Wrapper がすでに OS をまたいで固定してくれていた。
